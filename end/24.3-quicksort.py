...
def quicksort(list1):
    if len(list1)<=1:
        ...
        for e in list1:
            ...
            if e[0]>pivot[0]:
                right_sub.append(e)
        result=quicksort(right_sub) #so highest results come first
        result.append(pivot)
        result+=quicksort(left_sub)
        return result

def url_sort(possibilities,ranks):
    sorted_urls=[]
    sort_rank=[]
    for url in possibilities:
        sort_rank.append([ranks[url],url])
    sort_rank=quicksort(sort_rank)
    for i in range(0,len(sort_rank)):
        sorted_urls.append(sort_rank[i][1])
    return sorted_urls
...
def ordered_search(index, ranks, keyword):
    if keyword in index:
        possibilities=index[keyword]
        sorted_list=url_sort(possibilities,ranks)
        return sorted_list
    else:
        return None
